...
struct GridLayout {
    ...
    init(itemCount: Int, nearAspectRatio desiredAspectRatio: Double = 1, in size: CGSize) {
        ...
        for itemIndex in 1...itemCount {
            let columns = (itemCount / itemIndex) + (itemCount % itemIndex > 0 ? 1 : 0)
            if (itemIndex - 1) * columns < itemCount {
                let itemAspectRatio = sizeAspectRatio * (Double(itemIndex) / Double(columns))
                let variance = abs(itemAspectRatio - desiredAspectRatio)
                if smallestVariance == nil || variance < smallestVariance! {
                    smallestVariance = variance
                    bestLayout = (rowCount: itemIndex, columnCount: columns)
                }
            }
        }
